...
def apriori(X, minimal_support):
    ...
    # support_ = ...   # поддержка каждого 1-элементного набор
    support_ = np.array([0, 1, 0.82, 0.17])
    ...
    for k in range(2, n + 1):
        ...
        for candidate in multi_element_candidates:
            if support(X, candidate) > minimal_support:
                ...
                accepted_candidates = np.append(accepted_candidates, [candidate]).reshape((acceptedCount, 1))
